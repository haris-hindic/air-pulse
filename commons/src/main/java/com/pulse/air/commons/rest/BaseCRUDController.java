...
public class BaseCRUDController<TResponse, TRequest, TSearch extends BaseSearchRequest>
		extends BaseController<TResponse, TSearch> {

	public BaseCRUDController(final BaseCRUDService<TResponse, TRequest, TSearch> service) {
		super(service);
	}
	...
	public ApiResponse<TResponse> create(@Valid @RequestBody final TRequest request,
		...
		return ((BaseCRUDService<TResponse, TRequest, TSearch>) service).create(new ApiRequest<>(user, request));
	}
	...
	public ApiResponse<TResponse> create(@PathVariable final Long id, @Valid @RequestBody final TRequest request,
			...
			throws ApiException {
		return ((BaseCRUDService<TResponse, TRequest, TSearch>) service)
				.update(new ApiUpdateRequest<>(user, request, id));
	}

	@DeleteMapping(value = "{id}")
	public ApiResponse<String> delete(@PathVariable final Long id, @RequestHeader("AP_USER") final String user)
			throws ApiException {
		return ((BaseCRUDService<TResponse, TRequest, TSearch>) service).delete(new ApiRequest<>(user, id));
	}

	@DeleteMapping("/bulk-delete")
	public ApiResponse<String> bulkDelete(@RequestParam final List<Long> ids,
			@RequestHeader("AP_USER") final String user) throws ApiException {
		return ((BaseCRUDService<TResponse, TRequest, TSearch>) service).bulkDelete(new ApiListRequest<>(user, ids));
	}
}
